...
let spill_temps regs =
  let rtab = Hashtbl.create 10 in
  Hashtbl.iter (fun n t ->
      if !(t.t_refct) > 0 && t.t_reg <> R_none
          && List.mem t.t_reg regs then begin
        try
          let r = Hashtbl.find rtab t.t_reg in
          reserve_reg r; release_reg t.t_reg; t.t_reg <- r
        with Not_found ->
          let r = alloc stable in
          Hashtbl.add rtab t.t_reg r; move_reg r t.t_reg;
          release_reg t.t_reg; t.t_reg <- r
      end) temptab
